...
class DesktopSingleLocale(LocalesMixin, ReleaseMixin, MockMixin, PurgeMixin,
    ...
    def generate_complete_mar(self, locale):
        ...
        return_code = self._make(target=cmd,
                                 ...
                                 env=env)
        if return_code == 0:
            return_code == self._move_complete_mar(locale)
        return return_code

    def _move_complete_mar(self, locale):
        """moves the file generated by generate_complete_mar() into the right
           place"""
        # complete mar file is created in obj-l10n/dist/update
        # but we need it in obj-l10n-dist/current, let's move it
        ...
        if os.path.exists(dst_file):
            ...
            os.remove(dst_file)
        return self.move(src_file, dst_file)
